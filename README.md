...
2. **`[2] Patient Lookup`**: استعلام فوري للمريض برقم الملف/البطاقة (`Patient ID`) دون الحاجة لأي حساب أو كلمة مرور (محاكاة دقيقة لواقع العيادات الحقيقية).
3. **`[3] Exit`**: إغلاق النظام مع حفظ البيانات تلقائياً.
...
| **Patient** | *لا يوجد* (استعلام برقم `ID`) | *بدون كلمة مرور* | بوابة استعلام المريض (Patient Portal) برقم الملف فقط (عرض مواعيده، ترتيبه في الطابور، سجله الطبي). |
